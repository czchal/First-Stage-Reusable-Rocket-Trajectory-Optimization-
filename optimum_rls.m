function output = optimum_rls(vehicle)
global A T Isp initial drymass;
if vehicle=="Falcon 9 "
    A= pi*3.66^2/4;
    T= 5886000;
    Isp= 282;
...
elseif vehicle=="Electron "
    ...
    Isp=311;
    A= pi*1.2^2/4;
    initial= [94824,106485,1914,1463,3950];
    drymass=950; 
...
lb = [40.419323;200;300];
ub = [100;300;400];
x0 = [41,240,308]; % Initial guess
...
sol = ode15s(@dynamicsboostback,[0,xsolution(1)], initial,opts);   % boast-back phase 
...
sol2 = ode15s(@dynamicscoast,[xsolution(1),xsolution(2)], xinit,opts);  % coasting phase 
...
sol3 = ode15s(@dynamicslanding,[xsolution(2),xsolution(3)], xmid,opts);  % landing phase
...
xf= 0;     %The distance of the landing site from the take of site


sol = ode15s(@dynamicsboostback,[0,x(1)], initial);   % boast-back phase    state vector = [x,y,vx,vy,m]
...
sol2 = ode15s(@dynamicscoast,[x(1),x(2)], xinit);  % coasting phase      state vector = [x,y,v,gamma,m]

xmid=  [deval(sol2,x(2),1),deval(sol2,x(2),2),deval(sol2,x(2),3),deval(sol2,x(2),4),deval(sol2,x(2),5)];

sol3 = ode15s(@dynamicslanding,[x(2),x(3)], xmid);  % landing phase 
...
xf= 0;     %The distance of the landing site from the take off site


sol = ode15s(@dynamicsboostback,[0,x(1)], initial);   % boast-back phase 
...
sol2 = ode15s(@dynamicscoast,[x(1),x(2)], xinit);  % coasting phase 
...
f= -xland(5) + s1*(xland(1))^2 + s2*(xland(2))^2  + s3*(xland(3))^2 + s4*(xland(4))^2; %evaluate the cost function and output the resul
